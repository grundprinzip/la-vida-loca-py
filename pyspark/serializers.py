...
class SpecialLengths:
    ...
    # BEGIN-EDGE
    # A special marker for code literal Python functions.
    PYTHON_CODE_LITERAL = -30000
    # END-EDGE
...
class FramedSerializer(Serializer):
    ...
    def _read_with_length(self, stream):
        ...
        if length == SpecialLengths.END_OF_DATA_SECTION:
            ...
            return None
        # BEGIN-EDGE
        elif length == SpecialLengths.PYTHON_CODE_LITERAL:
            obj = self._read_func_from_code_literal(stream)
        # END-EDGE
        else:
    ...
    # BEGIN-EDGE
    def _read_func_from_code_literal(self, stream):
        """
        Read the function code literal and the return type (in JSON format) from the input stream.
        Return the pickled function command back.
        This simulates the process to provide a UDF via pyspark.
        """
        ...
        length = read_int(stream)
        obj = stream.read(length)
        code = obj.decode(encoding="utf-8")
        # Read return type
        length = read_int(stream)
        obj = stream.read(length)
        type = obj.decode(encoding="utf-8")

        # Generate a new module for the function
        # The function name `_udf_handler_func_entry` is assigned by the spark runtime.
        name = "_udf_handler_func_entry"
        module = types.ModuleType(name)
        # Evaluate the code
        exec(code, module.__dict__)
        sys.modules[name] = module

        return CloudPickleSerializer().dumps(
            (
                module._udf_handler_func_entry,
                _parse_datatype_json_string(type),
            )
        )
...
if sys.version_info < (3, 8):
    ...
    def _restore(module, name, fields, value):
        """Restore an object of namedtuple"""
        k = (module, name, fields)
        cls = __cls.get(k)
        if cls is None:
            cls = collections.namedtuple(name, fields, module=module)
            __cls[k] = cls
        return cls(*value)
    ...
    def _hack_namedtuple(cls):
        ...
        def __reduce__(self):
            return (_restore, (module, name, fields, tuple(self)))
    ...
    def _hijack_namedtuple():
        ...
        def namedtuple(*args, **kwargs):
            ...
            # [ML-12211] The module of the namedtuple class is determined based on the call stack.
            # We need to correct it because we inserted a frame with the hack.
            # Otherwise, pickling namedtuples does not work on the driver.
